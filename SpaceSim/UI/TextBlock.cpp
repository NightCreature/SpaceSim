...
void TextBlockInfo::CreateShaderSetup(Resource* resource)
{
	...
	mat.addTextureReference(Material::TextureSlotMapping((unsigned int)hashString(m_font->getPages().m_pages[0].m_fileName), Material::TextureSlotMapping::Diffuse ));//Requires we have a texture under the font name
