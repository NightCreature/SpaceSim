	...
	if( image.format == PF_R8G8B8 )
	{
		...
		else
		{
			while( cinfo.output_scanline < cinfo.output_height )
			{
				...
				// Convert the CMYK to RGB
				// Reference: http://www.easyrgb.com/index.php?X=MATH
				BYTE *outrow = &image.data[(cinfo.output_scanline-1) * image.pitch];
				BYTE *inrow = buffer[0];
				for( UINT x = 0; x < image.width; x++ )
				{
					// White is stored as 255,255,255,255 in the JPG file
					UINT K = inrow[0];
					UINT Y = inrow[1] * K / 255;
					UINT M = inrow[2] * K / 255;
					UINT C = inrow[3] * K / 255;
					outrow[0] = C; // red
					outrow[1] = Y; // green
					outrow[2] = M; // blue
					inrow  += 4;
